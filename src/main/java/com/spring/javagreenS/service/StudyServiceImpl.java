...
public class StudyServiceImpl implements StudyService {
	...
	public void setOperatorInputOk(OperatorVO vo) {
		// 1~50까지의난수를 구해서 해당 난수의 hashKey값을 operatorHashTable2에서 가져와서 연산시켜준다.
		...
		studydao.setOperatorInputOk(vo);
	...
	private String setPasswordEncoding(String pwd, int keyIdx) {
		...
		// 암호화를 위한 XOR 연산하기
		encPwd = intPwd ^ key;
		strPwd = String.valueOf(encPwd);// 암호화된 자료를 문자로변환한다
	...
	public String setOperatorSearch(OperatorVO vo) {
		...
		//검색된 아이디가존재한다면입력받은 pwd를 암호화해서다시 실제 DB의 pwd와 같은지를 판단
